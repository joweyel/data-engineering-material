...
FROM 
    `de-zoomcamp-26-485415.ny_taxi_hw3.external_yellow_tripdata`;
...
```sql
CREATE OR REPLACE TABLE `de-zoomcamp-26-485415.ny_taxi_hw3.yellow_tripdata_partitioned`
PARTITION BY DATE(tpep_dropoff_datetime) 
CLUSTER BY vendor_id
AS SELECT * FROM `de-zoomcamp-26-485415.ny_taxi_hw3.yellow_tripdata_materialized`;
```

**Result**
> "This statement created a new table named yellow_tripdata_partitioned."
...
```sql
-- Materialized (unpartitioned)
SELECT vendor_id FROM `de-zoomcamp-26-485415.ny_taxi_hw3.yellow_tripdata_materialized`
...
```sql
-- Partitioned
SELECT vendor_id FROM `de-zoomcamp-26-485415.ny_taxi_hw3.yellow_tripdata_partitioned`
WHERE DATE(tpep_dropoff_datetime) BETWEEN '2024-03-01' AND '2024-03-15';
```
...
```sql
SELECT COUNT(*) FROM `de-zoomcamp-26-485415.ny_taxi_hw3.yellow_tripdata_materialized`;
```

- **Bytes processed**: 0 B

There were no bytes processed since `COUNT(*)` gives you the overall number of rows, which is already stored in the metadata of the table, which is used instead of processing the entire table.